...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      dist: {
        files: [
          ...
          { expand: true, flatten: true, src: ['game/plugins/*.js'],
    ...
    jshint: {
      files: ['Gruntfile.js', 'game/*.js', 'game/**/*.js'],
      options: {
        'jshintrc' : 'game/.jshintrc',
      }
    }
  });
  ...
  grunt.registerTask('build', ['buildBootstrapper', 'jshint', 'browserify',
                               'copy']);
  grunt.registerTask('serve', ['build', 'connect:livereload', 'open', 'watch']);
  grunt.registerTask('default', ['serve']);
  grunt.registerTask('prod', ['build', 'copy']);

  grunt.registerTask('buildBootstrapper',
                     'builds the bootstrapper file correctly', function() {
    ...
    stateFiles.forEach(function(file) {
      ...
      if (!!state) {
        gameStates.push({shortName: state, stateName: _.capitalize(state) +
                         'State'});
      }
    });
